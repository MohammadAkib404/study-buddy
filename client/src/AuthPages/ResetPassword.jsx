...
export default function ResetPassword() {
 ...
 const checkOTP = async () => {
  ...
  const {data} = await axios.post(`${backendUrl}/verify-reset-otp`, {email, resetOtp});
  data.success? toast.success(data.message) : toast.error(data.message);
  console.log(data);
 }
 ...
 const changePassword = async (e) => {
  ...
  const {data} = await axios.post(`${backendUrl}/reset-password`, {email, resetOtp, newPassword});
  data.success? toast.success(data.message) : toast.error(data.message);
  console.log(data);
  // navigate("/");
 }
  ...
  return (
    <>
      ...
      <form className="bg-gray-50 w-6/10 h-max flex flex-col gap-8 p-10 rounded-xl">
        ...
        <button onClick={generateOTP} className="bg-brand text-xl text-base font-semibold p-3 rounded-lg">Submit</button>
      </form>
    </section>
    <section className='flex justify-center pt-30 w-full h-screen bg-gray-200'>
      ...
      <form className="bg-gray-50 w-6/10 h-max flex flex-col gap-8 p-10 rounded-xl">
        ...
        <button onClick={changePassword} className="bg-brand text-xl text-base font-semibold p-3 rounded-lg">Submit</button>
      </form>
    </section>
    </>
  )
