    ...
    return (
        <header className='fixed w-full z-50 flex px-7 md:px-12 py-3 bg-base text-primary border-b-1 border-border'>
            ...
            <div className={`bg-base sm:border-none border-2 border-border font-semibold absolute top-14 transition-all duration-300 ease-in-out ${menu ? 'h-auto opacity-100' : 'h-0 opacity-0'} left-0 w-screen sm:static p-6 sm:p-0 flex items-start gap-y-4 flex-col sm:flex-row sm:items-center sm:justify-end space-x-3 text-sm lg:text-[16px] list-none`}>
                ...
                <button className='ml-1 shadow-lg shadow-gray-500 sm:static absolute p-[6px] rounded-xl border border-border  top-4 right-7' onClick={() => setDarkTheme(!darkTheme)}>
