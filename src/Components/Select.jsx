...
function Select(props) {
    ...
    function SingleSelect() {
        return (
            <div className="w-8/10 relative">
                ...
                <div ref={optionsRef}>
                    <div
                        className='flex justify-between w-full p-3 border border-border rounded-lg'
                    ...
                    <ul className={`bg-base absolute w-full ${isMenuOpen ? 'scale-100 opacity-100 ' : 'scale-90 opacity-0 pointer-events-none'} z-99 transition-all duration-200 border border-border rounded-lg p-1 shadow-lg`}>
                        {props.options.map((opt, id) => (
                            <li
                                ...
                                onClick={() => { setSelectedOption(id); setIsMenuOpen(false) }}
                            >
                                <span>{opt}</span>
            ...
            <div ref={optionsRef}>
                ...
                >
                    ...
                    <div className='flex gap-2'>
                        {optionsSelected.map((selectedIndex) => (
                            <div className='flex gap-2.5 border border-border px-2 py-1 rounded-lg'>
    ...
    return (
        props.multiselect? <MultiSelect/> : <SingleSelect/>
    )
}
